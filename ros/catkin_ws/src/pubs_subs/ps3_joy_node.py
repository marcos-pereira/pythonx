...
def readWritePS3JoyCmds(ps3_joystick_object, cmd_vel_to_turtle_pub):	
	...
	#-------------------------------------------------------------------------
	# Some unused code which subscribe to other nodes
	#-------------------------------------------------------------------------
	...
	# Set subscriber to read PS3 joystick data
	# This info is used to debug in which position is each PS3 button and axis
	#-------------------------------------------------------------------------
	...
	if ps3_joystick_object.ps3_joystick_state.buttons[7] == 1:
		twist_msg_to_turtle.angular.z = 1*ps3_joystick_object.acceleration_gain;
	#-------------------------------------------------------------------------
	# Get PS3 analog axis sticks commands and write to twist msg to be published
	# For now, use the left stick to go forwards and backwards and 
	# use the right stick to turn left/right
	# Press R2/L2 to increase/decrease velocity
	#-------------------------------------------------------------------------
	# LEFT AXIS STICK - LEFTWARDS
	# if ps3_joystick_object.ps3_joystick_state.axes[0] != 0:
		# print "Left stick axis leftwards: ";
		# print ps3_joystick_object.ps3_joystick_state.axes[0];
		# twist_msg_to_turtle.linear.x = 1*ps3_joystick_object.acceleration_gain		
	# LEFT AXIS STICK - UPWARDS
	if ps3_joystick_object.ps3_joystick_state.axes[1] != 0:
		print "Left stick axis upwards: ";
		twist_msg_to_turtle.linear.x = ps3_joystick_object.ps3_joystick_state.axes[1]*ps3_joystick_object.acceleration_gain;
		print twist_msg_to_turtle.linear.x;
	# RIGHT AXIS STICK - LEFTWARDS
	if ps3_joystick_object.ps3_joystick_state.axes[2] != 0:
		print "Right stick axis leftwards: ";		 
		twist_msg_to_turtle.angular.z = ps3_joystick_object.ps3_joystick_state.axes[2];
		print twist_msg_to_turtle.angular.z;
	# RIGHT AXIS STICK - UPWARDS
	# if ps3_joystick_object.ps3_joystick_state.axes[3] != 0:		
		# print "Right stick axis upwards: ";				
		# twist_msg_to_turtle.angular.z = 1*ps3_joystick_object.acceleration_gain
		# print twist_msg_to_turtle.angular.z;
	
	#-------------------------------------------------------------------------
	# Make sure the twist linear and angular velocities which are not needed are zero
	#-------------------------------------------------------------------------
	...
	#-------------------------------------------------------------------------
	# Publish twist velocity command to turtle
	#-------------------------------------------------------------------------
...
def mainFunction():	
	...
	#-------------------------------------------------------------------------
	# PS3 joystick related
	# Subscribe to the joy node and instantiate PS3Joy object
	#-------------------------------------------------------------------------
	...
	#-------------------------------------------------------------------------
	# Loop which runs forever until program shutdown 
	# Subscribe to PS3 controller ROS node /joy
	# Wait for the setup time to end then
	# Call function which read and write data from/to the PS3 controller /joy ROS node
	#-------------------------------------------------------------------------
